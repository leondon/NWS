...
if ($_K ['theme']) {
 require S_ROOT.'/tpl/default/theme/'.$_K ['theme'].'/index.php';die;
	...
	$arrArticleTop = db_factory::get_one("select * from ".TABLEPRE."witkey_article where cat_type='article' and art_pic is not null order by pub_time desc limit 1",1,$intIndexCacheTime);
	$arrArticleLists = db_factory::query("select * from ".TABLEPRE."witkey_article where cat_type='article' order by pub_time desc limit 5",1,$intIndexCacheTime);
