<?php keke_tpl_class::checkrefresh('task/sreward/tpl/default/work', '1414472164' );?>  <ul class="nav nav-pills second-nav">
...
<div class="manuscripts">
    	<div  id="gj_summery">
    <?php if($arrWorkLists) { ?>
<?php if(is_array($arrWorkLists)) { foreach($arrWorkLists as $k => $v) { ?>
...
<?php } ?>
          ...
          <?php if($arrProcess_can['work_choose']) { ?>
          	<?php if(!in_array($v['work_status'],array('4','7','8'))) { ?>
            	<a href="javascript:chooseWork('<?php echo $v['work_id'];?>',4);void(0);"  class="btn btn-success btn-sm"><i class="fa fa-check-circle"></i> 设为中标</a>
             <?php } ?>
            <?php if(!in_array($v['work_status'],array('4','5','7','8'))) { ?>
            	<a href="javascript:chooseWork('<?php echo $v['work_id'];?>',5);void(0);"  class="btn btn-default btn-sm"><i class="fa fa-dot-circle-o"></i> 设为入围</a>
             <?php } ?>
            <?php if(!in_array($v['work_status'],array('4','7','8'))) { ?>
            	<a href="javascript:chooseWork('<?php echo $v['work_id'];?>',7);void(0);"  class="btn btn-default btn-sm"><i class="fa fa-times-circle"></i> 设为淘汰</a>
             <?php } ?>

             <?php } ?>
<!-- 				 <?php echo $arrProcess_can['work_vote']?>&&<?php echo $v['work_status']?>=='5'&&<?php echo $gUid?>!=<?php echo $v['uid']?>&&<?php echo $intCanVote?>>0 -->
<?php if($arrProcess_can['work_vote']&&$v['work_status']=='5'&&$gUid!=$v['uid']&&$intCanVote>0&&!$v['vote']) { ?>
            		<a href="javascript:voteWork('<?php echo $v['work_id'];?>');void(0);"  class="btn btn-default btn-sm"><i class="fa fa-gavel"></i> 投票</a>
            	<?php } ?>
           		<?php if($arrProcess_can['task_agree']&&$v['work_status']=='4'&&($uid==$arrTaskInfo['uid']||$v['uid']==$uid)) { ?>
           			<a href="index.php?do=agreement&taskId=<?php echo $v['task_id'];?>&agreeId=<?php echo $agree_id;?>" class="btn btn-default btn-sm"><i class="fa fa-times-circle"></i> 协议交付</a>
         		<?php } ?>
<?php if((!$mark[$v['work_id']]||($mark[$v['work_id']]['mark_status']!=1&&$mark[$v['work_id']]['mark_count']<2))&&!$check_right) { ?>
<?php if($arrProcess_can['work_mark']&&$v['work_status']=='4'&&$uid==$arrTaskInfo['uid']) { ?>
            	<span>
                	<?php if($agree_id) { ?><a class="btn btn-default btn-sm" href="index.php?do=agreement&taskId=<?php echo $v['task_id'];?>&agreeId=<?php echo $agree_id;?>"><?php if($mark[$v['work_id']]['mark_status']) { ?>修改评价<?php } else { ?>评价稿件<?php } ?></a><?php } ?>
                </span>
            <?php } ?>
<?php if($arrProcess_can['task_mark']&&$v['work_status']=='4'&&$uid==$v['uid']) { ?>
            	<span>
                	<?php if($agree_id) { ?><a class="btn btn-default btn-sm" href="index.php?do=agreement&taskId=<?php echo $v['task_id'];?>&agreeId=<?php echo $agree_id;?>"><?php if($mark[$v['work_id']]['mark_status']) { ?>修改评价<?php } else { ?>评价雇主<?php } ?></a><?php } ?>
               </span>
            <?php } ?>
            <?php } ?>
          </div>
...
<!-- sss   <?php echo $arrProcess_can['work_report']?>&&  eee-->
<?php if($arrProcess_can['work_report']&&$gUid!=$v['uid']&&$v['work_status']!=7&&$gUid) { ?>
            <li class="manuscript-meta-item">
...
<?php } ?>
  ...
  <div class="list-page">
    <div class="page-tips pull-left">
      显示 <?php echo $strPages['st'];?>~<?php echo $strPages['en'];?> 项 共 <?php echo $arrWorkDatas['count'];?> 个投稿
    </div>
