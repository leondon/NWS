<?php keke_tpl_class::checkrefresh('task/mreward/tpl/default/work', '1414474487' );?>
...
<?php if(is_array($arrWorkPrize)) { foreach($arrWorkPrize as $k => $v) { ?>
    <li <?php if($st==$k) { ?>class="active"<?php } ?>><a href="<?php echo $strUrl;?>&view=work&st=<?php echo $k?>#detail"><?php echo $v;?></a></li>
<?php } } ?>
...
<?php } ?>
  ...
  <?php $i=0; ?>
<?php if(is_array($arrWorkPrize)) { foreach($arrWorkPrize as $k1 => $v1) { ?>
<?php if(!in_array($v['work_status'],array('1','2','3','8'))) { ?>
<?php if($arrTaskPrize[$k1]['prize_count']>$arPrizeWorkDate['prize_'.$k1]) { ?>
                <a href="javascript:chooseWork('<?php echo $v['work_id'];?>','<?php echo $k1;?>',this);void(0);"   class="btn btn-success btn-sm"><i class="fa fa-check-circle"></i> <?php echo $v1;?></a>
 <?php } ?>
<?php } ?>
<?php if($k>0) { ?>
...
<?php if(!$arrMark[$v['work_id']]||($arrMark[$v['work_id']]['mark_status']!=1&&$arrMark[$v['work_id']]['mark_count']<2)) { ?>
          	 	<?php if($arrProcess_can['work_mark']&&(in_array($v['work_status'],array(1,2,3)))&&$uid==$arrTaskInfo['uid']) { ?> <a href="javascript:mark(2,'<?php echo $v['work_id'];?>','<?php echo $id;?>');void(0);"  class="btn btn-success btn-sm" ><?php if($arrMark[$v['work_id']]['mark_status']) { ?>修改评价<?php } else { ?>稿件评价<?php } ?></a></span><?php } ?>
<?php if($arrProcess_can['task_mark']&&(in_array($v['work_status'],array(1,2,3)))&&$uid==$v['uid']) { ?> <a href="javascript:mark(1,'<?php echo $v['work_id'];?>','<?php echo $id;?>');void(0);"  class="btn btn-success btn-sm" ><?php if($arrMark[$v['work_id']]['mark_status']) { ?>修改评价<?php } else { ?>评价雇主<?php } ?></a></span><?php } ?>
<?php } ?>
          </div>
