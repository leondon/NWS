<?php keke_tpl_class::checkrefresh('task/dtender/tpl/default/index', '1414480159' );?><!DOCTYPE HTML>
    ...
    <div class="detail-header">
        ...
        <div class="detail-ctrl">
              ...
              <?php if($arrProcess_can['work_hand']&&$gUid) { ?>
                <a href="index.php?do=taskhandle&op=planquote&taskId=<?php echo $arrTaskInfo['task_id'];?>" class="btn btn-primary btn-lg">我要报价</a>
              <?php } ?>
              ...
              <?php } ?>
 			 <?php if($arrProcess_can['task_pay']&&$gUid) { ?>
  <a href="index.php?do=taskhandle&op=consign&taskId=<?php echo $arrTaskInfo['task_id'];?>" class="btn btn-primary btn-lg">赏金托管</a>
  <?php } ?>
              </div>
        ...
        <div class="detail-header-body">
          <h1 class="detail-title"><span class="money">
            <sub>￥</sub><?php echo $arrCoverCash[$arrTaskInfo['task_cash_coverage']]['start_cove'];?>-<sub>￥</sub><?php echo $arrCoverCash[$arrTaskInfo['task_cash_coverage']]['end_cove'];?></span> <?php echo $arrTaskInfo['task_title'];?>
...
data-html="true" data-trigger="hover" data-content="单人中标模式"><?php echo $arrModelInfo['model_name'];?>模式 <i class="fa fa-angle-down"></i></a>
                <span><i class="fa fa-money"></i> <?php if($arrTaskInfo['task_status']) { ?> 已支付网站服务费用：<sub>￥</sub><?php echo $arrTaskInfo['real_cash'];?><?php } else { ?>未支付<?php } ?></span>
              </div>
            </div>
            <div class="detail-obj-item">
 ...
 <?php if($gUid !=$arrTaskInfo['uid']) { ?>
  ...
  <?php } ?>
    ...
    <div class="detail-progress">
        ...
        <li class="step-item <?php if($arrTaskInfo['task_status'] ==11) { ?>
<?php if($k <= 4) { ?>
action
<?php } ?>
<?php } else { ?>
<?php if($v['status'] <= $arrTaskInfo['task_status']) { ?>
action
<?php } ?>
<?php } ?>">
          <span class="step-num">
          	<?php if($arrTaskInfo['task_status'] ==11) { ?>
<?php if($k <4) { ?>
...
<?php } ?>
          ...
          <div class="step-text step-bottom">
            <p><strong class="step-title"><?php echo $v['desc'];?><?php if($arrTaskInfo['task_status'] ==11&&$v['status']==6) { ?>(交付仲裁中)<?php } ?></strong></p>
            <?php if($v['time']&& $arrTaskInfo['task_status'] >=$v['status']) { ?>
            	<p><?php echo date('Y-m-d',$v['time']) ?></p>
            <?php } ?>
          </div>
          <?php if(in_array($v['status'], array(2,3,4,5))&&$arrTaskInfo['task_status'] >=$v['status']) { ?>
    ...
    <div class="detail-body">
  	...
  	<?php if(is_array($arrTaskPics)) { foreach($arrTaskPics as $k => $v) { ?>
        <a class="detail-img-item" href="<?php echo $v?>" title="图片名称" rel="detail-img-group">
          <img src="<?php echo $v?>" alt="图片名称">
        </a>
<?php } } ?>
      </div>
...
<?php } } ?>
 ...
 <?php } ?>
  ...
  <!-- for-advertise end -->
    ...
    <a href="<?php echo $strUrl;?>&view=work#detail"    <?php if($view == 'work') { ?>    class="selected" <?php } ?>>稿件 <span class="badge"><?php echo $arrTaskInfo['work_num'];?></span></a>
    <a href="<?php echo $strUrl;?>&view=comment#detail" <?php if($view == 'comment') { ?> class="selected" <?php } ?>>留言 <span class="badge"><?php echo $arrTaskInfo['leave_num'];?></span></a>
    <a href="<?php echo $strUrl;?>&view=mark#detail"    <?php if($view == 'mark') { ?>    class="selected" <?php } ?>>评价 <span class="badge"><?php echo $arrTaskInfo['mark_num'];?></span></a>
  </div>
  <!-- tab end -->
