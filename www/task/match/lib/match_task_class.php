...
class match_task_class extends keke_task_class {
	...
	private function get_match_init() {
		...
		if ($match_info) {
			...
			$match_work = db_factory::get_one(sprintf("select * from %switkey_task_work where task_id='%d' and work_status=4",TABLEPRE,$this->_task_id));
			if($match_work){
				$work_detail = db_factory::get_one(sprintf("select * from %switkey_task_match_work where work_id='%d'",TABLEPRE,$match_work['work_id']));
	...
	public function work_bid($contact,$quote,$tar_content,$cycle) {
	global $_lang;
	...
	public function get_work($work_id) {
		return db_factory::get_one ( sprintf ( " select * from %switkey_task_work where work_id='%d'", TABLEPRE, $work_id ) );
	}
	...
	public function dispose_task() {
		...
		if ($this->set_task_status ( 8 )) {
			...
			$feed_arr = array ("feed_username" => array ("content" => $work_info ['username'], "url" => "index.php?do=seller&id={$work_info['uid']}" ), "action" => array ("content" => $_lang ['match_task_over'], "url" => "" ), "event" => array ("content" => $this->_task_title, "url" => $this->_task_url, 'cash' => $get_cash ) );
	...
	public function dispose_order($order_id) {
		...
		if ($order_info ['order_status'] == 'ok') {
			...
			if ($res) {
				...
				db_factory::execute ( sprintf ( " update %switkey_task set cash_cost='%s' where task_id='%d'", TABLEPRE,$task_info ['real_cash'], $this->_task_id ) );
				...
				db_factory::execute ( sprintf ( " update %switkey_task_match set deposit_cash='%.2f' where task_id='%d'", TABLEPRE, $task_info ['real_cash'], $this->_task_id ) );
